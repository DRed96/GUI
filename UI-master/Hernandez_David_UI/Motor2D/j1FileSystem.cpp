...
j1FileSystem::j1FileSystem() : j1Module()
{
	...
	//Github didn't save my project config, so i had to add this to locate config.xml :(
	AddPath("./../Game");
}
...
bool j1FileSystem::Awake(pugi::xml_node& config)
{
	...
	App->console->AddCommand(&c_AddDir, this);
...
bool j1FileSystem::AddDir(char* path)
{
	bool ret = true;
	...
	{
		//El error que se da al especificar un nombre de archivo incorrecto está incompleto.
		//Sobre el caracter 87, en lugar de una "a" de "archivo" PhysFS manda un /0.
		LOG("Error while creating new path %s.", path);
