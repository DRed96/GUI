...
bool j1App::Awake()
{
	...
	if (ret == true)
	{
		...
		{
			ret = item->data->PreAwake(config.child(item->data->name.GetString()));
	...
	if (ret == true)
	{
		...
		while (item != NULL && ret == true)
		{
			ret = item->data->PostAwake(config.child(item->data->name.GetString()));

			item = item->next;
		}
	}
	console->AddCVar("update_ms_alert", &msAlert);
...
bool j1App::DoUpdate()
{
	...
	if (showTimes)
		LOG("Determining modules update duration at frame %i. --------------------------------\n", frame_count);
