...
{
public:
	Command(const char* name, uint numberOfArguments){ commandName = name; nArgs = numberOfArguments; }
...
public:
	p2SString GetName() { return commandName; }
	uint GetNArgs() { return nArgs; }
	j1Module* GetListener() { return listener; }
	void SetListener(j1Module* _listener) { listener = _listener; }
...
#pragma region Commands
	...
	{
		Command_Save() : Command("save", 1){}
		void Function(p2DynArray<p2SString>* args)
		{
			p2SString tmp = *args->At(1);
			tmp += ".xml";
			App->SaveGame(tmp.GetString());
		}
	};
	Command_Save c_Save;
	...
	{
		Command_Load() : Command("load", 1){}
		void Function(p2DynArray<p2SString>* args)
		{
			p2SString tmp = *args->At(1);
			tmp += ".xml";
			App->LoadGame(tmp.GetString());
		}
	};
	Command_Load c_Load;
#pragma endregion
